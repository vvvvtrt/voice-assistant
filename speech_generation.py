...
def answers(message, next_len):
    global chat_history_ids

    new_user_input_ids = tokenizer.encode(f"|0|{get_length_param(message, tokenizer)}|" \
    ...
    return tokenizer.decode(chat_history_ids[:, input_len:][0], skip_special_tokens=True)

### INFERENCE

if __name__ == '__main__':
    print(answers("как у тебя дела?", "2"))
